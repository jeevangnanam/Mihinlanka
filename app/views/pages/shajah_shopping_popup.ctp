<html>
	...
	<body>
		<div style="width:100%;">
			...
			$dirname=APP.'webroot'.DS. 'img' . DS. 'dest_reviews' .DS .'places_for_shopping'.DS .'shajah'.DS ;//;
			$dh= opendir($dirname); 
			$files=array(); 
...
/* echo $this->Html->image('dest_reviews' .$s.'places_for_shopping'.$s.'gaya'.$s.$entry, 
 array('alt' => $entry,'class' => 'image_pr','rel' => 'lightbox'));//,'onclick'=>'loadRealSize(this)'*/
	...
	echo $this->Html->image('dest_reviews' .$s.'places_for_shopping'.$s.'shajah'.$s.$entry, array('alt' => $entry,'class' => 'image_pr')) ;
 	echo "<span>"; 
 	echo $this->Html->image('dest_reviews'.$s.'places_for_shopping'.$s.'shajah'.$s.$entry,array('alt' => $entry,'class' => 'image_pr'));
 	echo "</span></a>";			
					} 
